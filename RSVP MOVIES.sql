...
SELECT MONTH(date_published) AS months, COUNT(id) AS number_of_movies 
FROM movie
...
INNER JOIN movie AS mo
ON mo.id = g.movie_id
WHERE mo.country='USA' AND r.total_votes>1000 AND MONTH(date_published)=3 AND year=2017
GROUP BY g.genre
ORDER BY movie_count DESC;
